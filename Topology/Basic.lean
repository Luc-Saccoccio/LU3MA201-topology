...
-- j'ai du réécrire les lemme suivant avec (E : Set X) au lieu de {E : Set X} sinon cela me donne un message d'erreur

lemma closure_is_closed2 (E : Set X) : is_closed (Closure E) := inter_closed_is_closed {F : Set X | is_closed F ∧ E ⊆ F} fun _ => And.left
lemma sub_closure2 (E : Set X) : E ⊆ Closure E := Set.subset_sInter fun _ => And.right
lemma closure_closed_is_closed2 (E : Set X) (h : is_closed E) : Closure E = E :=
  Set.Subset.antisymm (closure_sub_closed h (Set.Subset.refl E)) sub_closure  
...
def strictement_croissant (f: ℕ → ℕ ): Prop :=  ∀ n: ℕ, ∀ m : Nat, n > m -> f n > f m 
...
lemma limite_suite_extraite (x : ℕ → X) (l : X) (f: ℕ → ℕ): lim x l ∧ strictement_croissant f -> lim (x ∘ f ) l := 
  by
    rintro ⟨hx, hf⟩ ε hε 
    obtain ⟨N, hN ⟩ := hx ε hε 
    use N
    ...
    have t : f n > f N := by  
      ...
      exact hn
    apply hN
    have sup_N: f N >= N := sorry -- il faut utiliser hf mais je ne sais pas comment
    linarith [t, sup_N]
...
def is_compact ( K : Set X) : Prop := ∀ x : ℕ → X, ∃ f : ℕ → ℕ, ∃ l ∈ K, strictement_croissant f ∧ converge_in K (x∘f ) l
...
lemma compact_is_closed : ∀  K : Set X , is_compact K → is_closed K :=
  by 
    ...
    have diff: Closure K ≠ K := by
      intro absurde 
      have j : is_closed (Closure K) := by exact closure_is_closed2 K 
      rw [absurde] at j
      apply h at j
      exact j

    have c : K ⊂ Closure K := 
      Set.ssubset_iff_subset_ne.mpr ⟨  sub_closure2 K ,  (diff.symm)⟩  
...
lemma subcompact_closed_is_compact (K H: Set X) (k_compact : is_compact K) (h_sub: H  ⊆ K) (h_closed : is_closed H)  : is_compact H := by
  ...
  rw [closure_closed_is_closed2 H] at l_in_h
  use f
  use l
